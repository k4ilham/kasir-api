...
func (repo *ProductRepository) GetAll() ([]models.Product, error) {
	...
	query := `
		SELECT p.id, p.name, p.price, p.stock, p.category_id, c.name as category_name, c.description as category_desc
		FROM products p
	...
	for rows.Next() {
		...
		err := rows.Scan(&p.ID, &p.Name, &p.Price, &p.Stock, &p.CategoryID, &catName, &catDesc)
		if err != nil {
			return nil, err
		}
...
func (repo *ProductRepository) GetByID(id int) (*models.Product, error) {
	...
	var p models.Product
	var catName, catDesc sql.NullString
	err := repo.db.QueryRow(query, id).Scan(&p.ID, &p.Name, &p.Price, &p.Stock, &p.CategoryID, &catName, &catDesc)
